...
sites[,habitat_id := .GRP, by = c("clc", "hlz", "bgr", "eunis_code")]
...
skip_types <- i.hold_j[n_cluster < 10] %>% unique(by = "habitat_id") %>% pull(habitat_id)
...
i.hold_j[, cluster_id := paste(habitat_id, cluster, sep = "_")]
i.hold_j[, cluster_id := .GRP, by = "cluster_id"]
...
singeltons <- i.hold_j[cluster_count == 1]
# - Some types would drop below the 10 sample threshold if we apply thinning. 
# - We collect them in a separate table 
rare_types <- i.hold_j[habitat_id %in% skip_types & cluster_id %in% singeltons$cluster_id]
...
thinned_sites <- rbind(manual, i.hold_j[site_id %in% min_dist$site_id])
